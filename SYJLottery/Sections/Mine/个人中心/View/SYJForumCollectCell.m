...
@implementation SYJForumCollectCell
...
- (void)setModel:(SYJForumCollectModel *)model{
    
    _model = model;
    self.title.text = model.authorName;
    ...
    if (model.isThumb == 0) {
        [self.favoriteBtn setImage:[UIImage imageNamed:@"点赞"] forState:UIControlStateNormal];
    }else{
        [self.favoriteBtn setImage:[UIImage imageNamed:@"点赞1"] forState:UIControlStateNormal];
    }
}
