...
@implementation PokerMovementsView
...
- (void)createUI
{
    ...
    for(int i = 0; i < _titelarray.count; i++)
    {
        ...
        btselete.tag=i;
        btselete.titleLabel.font=[UIFont systemFontOfSize:IPhone4_5_6_6P(12, 12, 13, 13)];
...
-(void)Switchingperformancedatawithtype:(NSString *)type{
    ...
    [[HttpManager manager] requestGetURL:HTTP_poker_trend withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
    ...
    for (int i = 0; i<array.count; i++) {
        ...
        for (int j = 0; j<numarr.count; j++) {
           ...
           lable.backgroundColor=[UIColor whiteColor];
            ...
            if ([lable.text isEqualToString:@"大"]||[lable.text isEqualToString:@"双"]||[lable.text isEqualToString:@"龙"]||[lable.text isEqualToString:@"红波"]||[lable.text isEqualToString:@"极大"]) {
                lable.textColor=RedColor;
            }else if ([lable.text isEqualToString:@"小"]||[lable.text isEqualToString:@"单"]||[lable.text isEqualToString:@"虎"]||[lable.text isEqualToString:@"绿波"]||[lable.text isEqualToString:@"极小"]){
