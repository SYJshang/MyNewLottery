...
#import "BetModel.h"
#import "LuckMovementsView.h"
#import "WebViewController.h"
//#import "DepositViewController.h"

#import "PokerMovementsView.h"

@interface HappyPokerController ()<DropDownViewDelegate,BetFootViewDelegate,BetHeadViewDelegate,UIScrollViewDelegate,BetTypeHeadViewDelegate,SettlementViewDelegate,OptionalViewDelegate,QuickViewDelegate,BetDetailHeadViewDelegate,UITextFieldDelegate>
{
    UIView          *_TopView;               //导航栏
...
@property (nonatomic, strong) PokerMovementsView     *movementsview;         //走势视图
...
@implementation HappyPokerController
- (instancetype)init {
    self = [super init];
    if (self) {
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [MyConfig currentConfig].isback=@"9";
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.array = [NSArray arrayWithObjects:@"两面盘",@"特码",nil];
    [self addTopView];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(keyboardDidHide:)
                                               ...
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(isbackrefresh) name:@"pkNotifica" object:nil];
...
-(void)getcurrentTimedata{
    ...
    DLog(@"=------%@-----%i",[MyConfig currentConfig].isback,_isrun);
    
    if (![[MyConfig currentConfig].isback isEqualToString:@"9"]&&_isrun==0) {
        return;
    }
    ...
    [[HttpManager manager] requestGetURL:HTTP_poker_currentTime withParams:dict withTarget:self withHud:NO withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
-(void)getrankingdata{
    ...
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    [dict setValue:[MyConfig currentConfig].userU forKey:@"u"];//[可选]用户key
    [[HttpManager manager] requestGetURL:HTTP_poker_hotRanking withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
        if ([responseObject[@"code"] isEqualToString:@"200"]) {
...
-(void)getbetPaneldata{
    ...
    [[HttpManager manager] requestGetURL:HTTP_poker_betPanel withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
-(void)setviewfrem{
    ...
    DLog(@"-=-=------%f---------%f",_twohead.height,_thrhead.height);
}
...
#pragma mark - 点击规则
-(void)clickrules{
    [[HttpManager manager] requestGetURL:HTTP_poker_rules withParams:nil withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
-(BetDetailHeadView*)headview{
    if (!_headview) {
        ...
        _headview.lblottery.attributedText = [[NSString stringWithFormat:@"快乐扑克3<u>   </u><v>000000期</v>"] attributedStringWithStyleBook:style];
...
//走势
-(PokerMovementsView*)movementsview{
    if (!_movementsview)
    {
        _movementsview=[[PokerMovementsView alloc]initWithRightdistance:80];
...
-(void)clickbetwithmoney:(NSString *)money{
    ...
    if ([_isweb isEqualToString:@"1"]) {
        ...
        NSString *strUrl = [_weburl stringByReplacingOccurrencesOfString:@"t=" withString:@"t=8"];
    ...
    [[HttpManager manager] requestGetURL:HTTP_poker_bet withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
