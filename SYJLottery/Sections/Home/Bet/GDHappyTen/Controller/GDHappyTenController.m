...
#import "BetModel.h"
#import "GDMovementsView.h"
#import "WebViewController.h"
//#import "DepositViewController.h"
@interface GDHappyTenController ()<DropDownViewDelegate,BetFootViewDelegate,BetHeadViewDelegate,UIScrollViewDelegate,BetTypeHeadViewDelegate,SettlementViewDelegate,OptionalViewDelegate,QuickViewDelegate,BetDetailHeadViewDelegate,UITextFieldDelegate>
{
    UIView          *_TopView;               //导航栏
...
@property (nonatomic, strong) GDMovementsView       *movementsview;         //走势视图
...
@implementation GDHappyTenController
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [MyConfig currentConfig].isback=@"6";
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.array = [NSArray arrayWithObjects:@"两面盘",@"第1球",@"第2球",@"第3球",@"第4球",@"第5球",@"第6球",@"第7球",@"第8球",nil];
    [self addTopView];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(keyboardDidHide:)
                                               ...
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(isbackrefresh) name:@"gdNotifica" object:nil];
...
-(void)getcurrentTimedata{
    if (![[MyConfig currentConfig].isback isEqualToString:@"6"]&&_isrun==0) {
        return;
    }
    ...
    [[HttpManager manager] requestGetURL:HTTP_gdk10_currentTime withParams:dict withTarget:self withHud:NO withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
-(void)getrankingdata{
    ...
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    [dict setValue:[MyConfig currentConfig].userU forKey:@"u"];//[可选]用户key
    [[HttpManager manager] requestGetURL:HTTP_gdk10_hotRanking withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
-(void)getbetPaneldata{
    ...
    [[HttpManager manager] requestGetURL:HTTP_gdk10_betPanel withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
-(void)setViewlayout{
    ...
    if (_playType==0) {//两面盘
        ...
        _onehead.viewtag=1;
        NSString*titlestr;
        titlestr= [_array objectAtIndex:_playType];
        _onehead.titleArray=@[titlestr];
        [_scrollview addSubview:_onehead];
...
#pragma mark - 点击规则
-(void)clickrules{
    [[HttpManager manager] requestGetURL:HTTP_gdk10_rules withParams:nil withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
-(BetDetailHeadView*)headview{
    if (!_headview) {
        ...
        _headview.lblottery.attributedText = [[NSString stringWithFormat:@"广东快乐十分<u>   </u><v>000000期</v>"] attributedStringWithStyleBook:style];
...
//走势
-(GDMovementsView*)movementsview{
    if (!_movementsview)
    {
        _movementsview=[[GDMovementsView alloc]initWithRightdistance:80];
...
-(void)clickbetwithmoney:(NSString *)money{
    ...
    if ([_isweb isEqualToString:@"1"]) {
        ...
        NSString *strUrl = [_weburl stringByReplacingOccurrencesOfString:@"t=" withString:@"t=5"];
    ...
    }else{    [[HttpManager manager] requestGetURL:HTTP_gdk10_bet withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
