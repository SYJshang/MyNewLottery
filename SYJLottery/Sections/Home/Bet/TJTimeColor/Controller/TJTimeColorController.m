...
#import "BetModel.h"
#import "TJMovementsView.h"
#import "WebViewController.h"
//#import "DepositViewController.h"

@interface TJTimeColorController ()<DropDownViewDelegate,BetFootViewDelegate,BetHeadViewDelegate,UIScrollViewDelegate,BetTypeHeadViewDelegate,SettlementViewDelegate,OptionalViewDelegate,QuickViewDelegate,BetDetailHeadViewDelegate,UITextFieldDelegate>
{
    UIView          *_TopView;               //导航栏
...
@property (nonatomic, strong) TJMovementsView       *movementsview;         //走势视图
...
@implementation TJTimeColorController
- (instancetype)init {
    self = [super init];
    if (self) {
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [MyConfig currentConfig].isback=@"7";
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(keyboardDidHide:)
                                               ...
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(isbackrefresh) name:@"tjNotifica" object:nil];
...
-(void)getcurrentTimedata{
    if (![[MyConfig currentConfig].isback isEqualToString:@"7"]&&_isrun==0) {
        return;
    }
    ...
    [[HttpManager manager] requestGetURL:HTTP_tjSsc_currentTime withParams:dict withTarget:self withHud:NO withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
-(void)getrankingdata{
    ...
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    [dict setValue:[MyConfig currentConfig].userU forKey:@"u"];//[可选]用户key
    [[HttpManager manager] requestGetURL:HTTP_tjSsc_hotRanking withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
-(void)getbetPaneldata{
    ...
    [[HttpManager manager] requestGetURL:HTTP_tjSsc_betPanel withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
#pragma mark - 点击规则
-(void)clickrules{
    [[HttpManager manager] requestGetURL:HTTP_tjSsc_rules withParams:nil withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
-(BetDetailHeadView*)headview{
    if (!_headview) {
        ...
        _headview.lblottery.attributedText = [[NSString stringWithFormat:@"天津时时彩<u>   </u><v>000000期</v>"] attributedStringWithStyleBook:style];
...
//走势
-(TJMovementsView*)movementsview{
    if (!_movementsview)
    {
        _movementsview=[[TJMovementsView alloc]initWithRightdistance:80];
...
//        NSString *strUrl = [_weburl stringByReplacingOccurrencesOfString:@"t=" withString:@"t=6"];
...
//        
        ...
        [[HttpManager manager] requestGetURL:HTTP_tjSsc_bet withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
