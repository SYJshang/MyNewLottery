...
#import "BetModel.h"
#import "JSThreeMovementsView.h"
#import "WebViewController.h"
//#import "DepositViewController.h"
@interface JSThreeController ()<DropDownViewDelegate,BetFootViewDelegate,BetHeadViewDelegate,UIScrollViewDelegate,BetTypeHeadViewDelegate,SettlementViewDelegate,OptionalViewDelegate,QuickViewDelegate,BetDetailHeadViewDelegate,UITextFieldDelegate>
{
    UIView          *_TopView;               //导航栏
...
@property (nonatomic, strong) JSThreeMovementsView       *movementsview;         //走势视图
...
@implementation JSThreeController
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [MyConfig currentConfig].isback=@"11";
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.array = [NSArray arrayWithObjects:@"两面盘",@"两连",nil];
    [self addTopView];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(keyboardDidHide:)
                                               ...
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(isbackrefresh) name:@"JSNotifica" object:nil];
...
-(void)getcurrentTimedata{
    if (![[MyConfig currentConfig].isback isEqualToString:@"11"]&&_isrun==0) {
        return;
    }
    ...
    [[HttpManager manager] requestGetURL:HTTP_jsK3_currentTime withParams:dict withTarget:self withHud:NO withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
-(void)getrankingdata{
    ...
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    [dict setValue:[MyConfig currentConfig].userU forKey:@"u"];//[可选]用户key
    [[HttpManager manager] requestGetURL:HTTP_jsK3_hotRanking withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
-(void)getbetPaneldata{
    ...
    [[HttpManager manager] requestGetURL:HTTP_jsK3_betPanel withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
-(void)setviewfrem{
    ...
    if (_playType==1) {
        ...
        _optionaltwo.frame=CGRectMake(0, _onehead.bottom+_optionalone.height, MSW, 0);
        _quicktwo.frame=_optionaltwo.frame;
        _thrhead.frame=CGRectMake(0, _onehead.bottom+_optionalone.height, MSW, 0);
        _optionalthr.frame=CGRectMake(0, _onehead.bottom+_optionalone.height, MSW, 0);
        _quickthr.frame=_optionalthr.frame;
        _fourhead.frame=CGRectMake(0, _onehead.bottom+_optionalone.height, MSW, 0);
        _optionalfour.frame=CGRectMake(0, _onehead.bottom+_optionalone.height, MSW, 0);
        _quickfour.frame=_optionalfour.frame;
    ...
    _scrollview.contentSize = CGSizeMake(MSW,_betheadview.height+_onehead.height+_twohead.height+_thrhead.height+_fourhead.height+_optionalone.height+_optionaltwo.height+_optionalthr.height+_optionalfour.height);
}
...
#pragma mark - 点击规则
-(void)clickrules{
    [[HttpManager manager] requestGetURL:HTTP_jsK3_rules withParams:nil withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        if ([responseObject[@"code"] isEqualToString:@"200"]) {
...
-(BetDetailHeadView*)headview{
    if (!_headview) {
        ...
        _headview.lblottery.attributedText = [[NSString stringWithFormat:@"江苏快3<u>   </u><v>000000期</v>"] attributedStringWithStyleBook:style];
...
//走势
-(JSThreeMovementsView*)movementsview{
    if (!_movementsview)
    {
        _movementsview=[[JSThreeMovementsView alloc]initWithRightdistance:80];
...
-(void)clickbetwithmoney:(NSString *)money{
    ...
    if ([_isweb isEqualToString:@"1"]) {
        ...
        NSString *strUrl = [_weburl stringByReplacingOccurrencesOfString:@"t=" withString:@"t=10"];
    ...
    [[HttpManager manager] requestGetURL:HTTP_jsK3_bet withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
