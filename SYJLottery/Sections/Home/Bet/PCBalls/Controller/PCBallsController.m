...
@interface PCBallsController ()<DropDownViewDelegate,BetFootViewDelegate,BetHeadViewDelegate,UIScrollViewDelegate,BetTypeHeadViewDelegate,SettlementViewDelegate,OptionalViewDelegate,QuickViewDelegate,BetDetailHeadViewDelegate,UITextFieldDelegate>
{
    UIView          *_TopView;               //导航栏
...
@property (nonatomic, strong) PCMovementsView       *movementsview;         //走势视图
...
@implementation PCBallsController
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [MyConfig currentConfig].isback=@"5";
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(keyboardDidHide:)
                                               ...
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(isbackrefresh) name:@"pcNotifica" object:nil];
...
-(void)getcurrentTimedata{
    if (![[MyConfig currentConfig].isback isEqualToString:@"5"]&&_isrun==0) {
        return;
    }
    ...
    [[HttpManager manager] requestGetURL:HTTP_bjLu28_currentTime withParams:dict withTarget:self withHud:NO withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
-(void)getrankingdata{
    ...
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    [dict setValue:[MyConfig currentConfig].userU forKey:@"u"];//[可选]用户key
    [[HttpManager manager] requestGetURL:HTTP_bjLu28_hotRanking withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
-(void)getbetPaneldata{
    ...
    [[HttpManager manager] requestGetURL:HTTP_bjLu28_betPanel withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
#pragma mark - 点击规则
-(void)clickrules{
    [[HttpManager manager] requestGetURL:HTTP_bjLu28_rules withParams:nil withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        if ([responseObject[@"code"] isEqualToString:@"200"]) {
...
-(BetDetailHeadView*)headview{
    if (!_headview) {
        ...
        _headview.lblottery.attributedText = [[NSString stringWithFormat:@"PC蛋蛋<u>   </u><v>000000期</v>"] attributedStringWithStyleBook:style];
...
//走势
-(PCMovementsView*)movementsview{
    if (!_movementsview)
    {
        _movementsview=[[PCMovementsView alloc]initWithRightdistance:80];
...
-(void)clickbetwithmoney:(NSString *)money{
    ...
    if ([_isweb isEqualToString:@"1"]) {
        ...
        NSString *strUrl = [_weburl stringByReplacingOccurrencesOfString:@"t=" withString:@"t=4"];
    ...
    [[HttpManager manager] requestGetURL:HTTP_bjLu28_bet withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
