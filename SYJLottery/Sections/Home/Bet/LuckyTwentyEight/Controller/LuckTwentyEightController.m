...
@interface LuckTwentyEightController ()<DropDownViewDelegate,BetFootViewDelegate,BetHeadViewDelegate,UIScrollViewDelegate,BetTypeHeadViewDelegate,SettlementViewDelegate,OptionalViewDelegate,QuickViewDelegate,BetDetailHeadViewDelegate,UITextFieldDelegate>
{
    UIView          *_TopView;               //导航栏
...
@property (nonatomic, strong) LuckMovementsView     *movementsview;         //走势视图
...
@implementation LuckTwentyEightController
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [MyConfig currentConfig].isback=@"3";
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(keyboardDidHide:)
                                               ...
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(isbackrefresh) name:@"luckNotifica" object:nil];
...
-(void)getcurrentTimedata{
    ...
    if (![[MyConfig currentConfig].isback isEqualToString:@"3"]&&_isrun==0) {
        return;
    }
    
    DLog(@"-=-------%@---",[MyConfig currentConfig].isback);
    ...
    [[HttpManager manager] requestGetURL:HTTP_xjpLu28_currentTime withParams:dict withTarget:self withHud:NO withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
-(void)getrankingdata{
    ...
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    [dict setValue:[MyConfig currentConfig].userU forKey:@"u"];//[可选]用户key
    [[HttpManager manager] requestGetURL:HTTP_xjpLu28_hotRankingg withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
-(void)getbetPaneldata{
    ...
    [[HttpManager manager] requestGetURL:HTTP_xjpLu28_betPanel withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
#pragma mark - 点击规则
-(void)clickrules{
    [[HttpManager manager] requestGetURL:HTTP_xjpLu28_rules withParams:nil withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
-(BetDetailHeadView*)headview{
    if (!_headview) {
        ...
        _headview.lblottery.attributedText = [[NSString stringWithFormat:@"幸运28<u>   </u><v>000000期</v>"] attributedStringWithStyleBook:style];
...
//走势
-(LuckMovementsView*)movementsview{
    if (!_movementsview)
    {
        _movementsview=[[LuckMovementsView alloc]initWithRightdistance:80];
...
//        NSString *strUrl = [_weburl stringByReplacingOccurrencesOfString:@"t=" withString:@"t=2"];
...
//        [ [ UIApplication sharedApplication]openURL:[NSURL URLWithString:strUrl2]];
    ...
    [[HttpManager manager] requestGetURL:HTTP_xjpLu28_bet withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
