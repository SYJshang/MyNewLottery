...
@interface SixLotteryMovementsView ()
{
...
@implementation SixLotteryMovementsView
...
-(void)Switchingperformancedatawithtype:(NSString *)type{
    ...
    [dict setValue:type forKey:@"type"];//[必选]类型 0=号码 1=大小 2=单双 3=和
    [[HttpManager manager] requestGetURL:HTTP_markSix_trend withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
-(void)setviewlayoutwitharray:(NSArray*)array andtype:(NSString*)type{
    ...
    for (int i = 0; i<array.count; i++) {
        ...
        for (int j = 0; j<numarr.count; j++) {
            ...
            if ([lable.text isEqualToString:@"大"]||[lable.text isEqualToString:@"双"]||[lable.text isEqualToString:@"龙"]) {
                lable.textColor=RedColor;
            }else if ([lable.text isEqualToString:@"小"]||[lable.text isEqualToString:@"单"]||[lable.text isEqualToString:@"虎"]){
