//
//  SixLotteryController.m
//  longzhicai
//
//  Created by lili on 2017/6/5.
...
#import "BetModel.h"
#import "SixLotteryMovementsView.h"
#import "WebViewController.h"
//#import "DepositViewController.h"
@interface SixLotteryController ()<DropDownViewDelegate,BetFootViewDelegate,SixBetHeadViewDelegate,UIScrollViewDelegate,BetTypeHeadViewDelegate,SettlementViewDelegate,OptionalViewDelegate,QuickViewDelegate,BetDetailHeadViewDelegate,UITextFieldDelegate>
{
    UIView          *_TopView;               //导航栏
    ...
    NSString        * _quickBet;                   //[data]是否只能快捷下载 0可以自选下载，1必须快捷下注
    NSString        * _minBet;                   //[data]每次下注最少必须投注多少项。
    NSString        * _maxBet;                   //[data]每次下注最大必须投注多少项
    NSString        * _combination;              //[data]是否需要排列组合， 0= 不需要， 1=必须排列组合
...
@property (nonatomic, strong) SixLotteryMovementsView       *movementsview;         //走势视图
@property (nonatomic, strong) RankingView           *rangkingview;          //冷热排行
@property (nonatomic, strong) UIScrollView          *scrollview;            //主滑动视图
@property (nonatomic, strong) SixBetHeadView           *betheadview;           //上期开奖结果
...
@implementation SixLotteryController
- (instancetype)init {
    self = [super init];
    if (self) {
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [MyConfig currentConfig].isback=@"13";
...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.array = [NSArray arrayWithObjects:@"特码A",@"特码B",@"正码",@"正1特",@"正2特",@"正3特",@"正4特",@"正5特",@"正6特",@"正码1-6",@"过关",@"二全中",@"二中特",@"特串",@"三全中",@"三中二",@"四全中",@"半波",@"一肖",@"尾数",@"特码生肖",@"二肖",@"三肖",@"四肖",@"五肖",@"六肖",@"七肖",@"八肖",@"九肖",@"十肖",@"十一肖",@"二肖连中",@"三肖连中",@"四肖连中",@"五肖连中",@"二肖连不中",@"三肖连不中",@"四肖连不中",@"二尾连中",@"三尾连中",@"四尾连中",@"二尾连不中",@"三尾连不中",@"四尾连不中",@"五不中",@"六不中",@"七不中",@"八不中",@"九不中",@"十不中",@"十一不中",@"十二不中",nil];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(keyboardDidHide:)
                                               ...
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(isbackrefresh) name:@"sixNotifica" object:nil];
...
-(void)getcurrentTimedata{
    if (![[MyConfig currentConfig].isback isEqualToString:@"13"]&&_isrun==0) {
        return;
    }
    ...
    [[HttpManager manager] requestGetURL:HTTP_markSix_currentTime withParams:dict withTarget:self withHud:NO withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
-(void)getrankingdata{
    ...
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    [dict setValue:[MyConfig currentConfig].userU forKey:@"u"];//[可选]用户key
    [[HttpManager manager] requestGetURL:HTTP_markSix_hotRanking withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
//    HTTP_gdPick11_betPanel     HTTP_markSix_betPanel
    [[HttpManager manager] requestGetURL:HTTP_markSix_betPanel withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@------", responseObject)
        
        if ([responseObject[@"code"] isEqualToString:@"200"]) {
            NSArray * items = responseObject[@"data"][@"betItems"];
            _betheadview.bettype=responseObject[@"data"][@"quickBet"];
            _quickBet=responseObject[@"data"][@"quickBet"];
            _minBet=responseObject[@"data"][@"minBet"];
            _maxBet=responseObject[@"data"][@"maxBet"];
            _combination=responseObject[@"data"][@"combination"];
            
            if ([_quickBet isEqualToString:@"0"]) {
                _bettype=1;
            }else{
...
-(void)setViewlayout{
    ...
    if (_playType==0||_playType==2) {//特码A",@"特码B"
        
        DLog(@"-=-=----%li",_betpanelarray.count);

        
        _onehead=[[BetTypeHeadView alloc]initWithFrame:CGRectMake(0,_betheadview.bottom, MSW, BetHeadHight)];
        _onehead.delegate=self;
        _onehead.viewtag=1;
        BetPanelModel*modeltitle=[_betpanelarray objectAtIndex:0];
        _onehead.titleArray=@[modeltitle.optionTitle];
        ...
        BetPanelModel*modeltitle2=[_betpanelarray objectAtIndex:1];
        _twohead.titleArray=@[modeltitle2.optionTitle];
    ...
    }else if (_playType==1||_playType==3||_playType==4||_playType==5||_playType==6||_playType==7||_playType==8||_playType==11||_playType==12||_playType==13||_playType==14||_playType==15||_playType==16||_playType==17||_playType==18||_playType==19||_playType==20||_playType==21||_playType==22||_playType==23||_playType==24||_playType==25||_playType==26||_playType==27||_playType==28||_playType==29||_playType==30||_playType==31||_playType==32||_playType==33||_playType==34||_playType==35||_playType==36|_playType==37||_playType==38||_playType==39||_playType==40||_playType==41||_playType==42||_playType==43||_playType==44||_playType==45||_playType==46||_playType==47||_playType==48||_playType==49||_playType==50||_playType==51){
    ...
    }else if (_playType==9||_playType==10){//正码1-6
    
        _onehead=[[BetTypeHeadView alloc]initWithFrame:CGRectMake(0,_betheadview.bottom, MSW, BetHeadHight)];
        _onehead.delegate=self;
        _onehead.viewtag=1;
        _onehead.titleArray=@[@"正码1"];
        [_scrollview addSubview:_onehead];
        
        _twohead=[[BetTypeHeadView alloc]initWithFrame:CGRectMake(0, _onehead.bottom, MSW, BetHeadHight)];
        _twohead.titleArray=@[@"正码2"];
        _twohead.delegate=self;
        _twohead.viewtag=2;
        [_scrollview addSubview:_twohead];
        
        _thrhead=[[BetTypeHeadView alloc]initWithFrame:CGRectMake(0, _twohead.bottom, MSW, BetHeadHight)];
        _thrhead.titleArray=@[@"正码3"];
        ...
        _twohead=[[BetTypeHeadView alloc]initWithFrame:CGRectMake(0, _onehead.bottom, MSW, BetHeadHight)];
        _twohead.titleArray=@[@"第1球",@"第2球",@"第3球",@"第4球"];
        _twohead.delegate=self;
        _twohead.viewtag=2;
        [_scrollview addSubview:_twohead];
        
        _thrhead=[[BetTypeHeadView alloc]initWithFrame:CGRectMake(0, _twohead.bottom, MSW, BetHeadHight)];
        _thrhead.titleArray=@[@"第5球",@"第6球",@"第7球",@"第8球"];
        _thrhead.delegate=self;
        _thrhead.viewtag=3;
        [_scrollview addSubview:_thrhead];
        ...
        if (_betpanelarray.count>0) {
            ...
            BetPanelModel*model=[_betpanelarray objectAtIndex:0];
            BetPanelModel*model2=[_betpanelarray objectAtIndex:1];
            BetPanelModel*model3=[_betpanelarray objectAtIndex:5];
            _onegrouphght=(int)((model.optionItems.count-1)/4+1)*50+10;
        ...
        for (int i=0; i<4; i++) {
            BetPanelModel*onemodel=[_betpanelarray objectAtIndex:i%4+1];
            NSMutableArray *array = [NSMutableArray array];
            array = [BetModel mj_objectArrayWithKeyValuesArray:onemodel.optionItems];
            for (int j = 0; j<onemodel.optionItems.count; j++) {
        ...
        for (int i=0; i<4; i++) {
            BetPanelModel*onemodel=[_betpanelarray objectAtIndex:i%4+5];
            NSMutableArray *array = [NSMutableArray array];
            array = [BetModel mj_objectArrayWithKeyValuesArray:onemodel.optionItems];
            for (int j = 0; j<onemodel.optionItems.count; j++) {
                OptionalView*option = [[OptionalView alloc]initWithFrame:CGRectMake((W)*(i%4),j * (H), W, H)];
...
-(void)setviewfrem{
    ...
    if (_tworouphght==0) {
        ...
        _optionaltwo.frame=CGRectMake(0, _twohead.bottom, MSW, 0);
        _quicktwo.frame=_optionaltwo.frame;
    }
    if (_thrgrouphght==0){
        _thrhead.frame=CGRectMake(0,_twohead.bottom+_optionaltwo.height, MSW, 0);
        _optionalthr.frame=CGRectMake(0, _thrhead.bottom, MSW, 0);
        _quickthr.frame=_optionalthr.frame;
        
    }
    if (_fourgrouphght==0){
        _fourhead.frame=CGRectMake(0,_thrhead.bottom+_optionalthr.height, MSW, 0);
        _optionalfour.frame=CGRectMake(0, _fourhead.bottom, MSW, 0);
        _quickfour.frame=_optionalfour.frame;
    }
    if (_fivegouphght==0){
        _fivehead.frame=CGRectMake(0,_fourhead.bottom+_optionalfour.height, MSW, 0);
        _optionalfive.frame=CGRectMake(0, _fivehead.bottom, MSW, 0);
        _quicktfive.frame=_optionalfive.frame;
...
#pragma mark - 点击规则
-(void)clickrules{
    [[HttpManager manager] requestGetURL:HTTP_markSix_rules withParams:nil withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        if ([responseObject[@"code"] isEqualToString:@"200"]) {
...
-(BetDetailHeadView*)headview{
    if (!_headview) {
        ...
        _headview.lblottery.attributedText = [[NSString stringWithFormat:@"六合彩u>   </u><v>000000期</v>"] attributedStringWithStyleBook:style];
...
//走势
-(SixLotteryMovementsView*)movementsview{
    if (!_movementsview)
    {
        _movementsview=[[SixLotteryMovementsView alloc]initWithRightdistance:80];
...
//上期开奖结果
-(SixBetHeadView*)betheadview{
    if (!_betheadview) {
        _betheadview=[[SixBetHeadView alloc]initWithFrame:CGRectMake(0,0, MSW, 120)];
        _betheadview.backgroundColor=[UIColor whiteColor];
        _betheadview.delegate=self;
        _betheadview.lotterytype=@"12";
...
-(void)clickbetwithmoney:(NSString *)money{
    ...
    if ([_isweb isEqualToString:@"1"]) {
        ...
        NSString *strUrl = [_weburl stringByReplacingOccurrencesOfString:@"t=" withString:@"t=12"];
    ...
    [[HttpManager manager] requestGetURL:HTTP_markSix_bet withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
...
-(void)combinaInfo{
    ...
    for (int i=0; i<_seletequickarray.count; i++) {
        BetModel*model=[_seletequickarray objectAtIndex:i];
        if (i==0) {
            opidstr=[NSString stringWithFormat:@"%@",model.optionId];
        }else{
        opidstr=[NSString stringWithFormat:@"%@,%@",opidstr,model.optionId];
    }
    }
    
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    [dict setValue:opidstr forKey:@"optionId"];//[必选]投注的项的id拼接的字符串。例如 1,5,8
    [dict setValue:@"12" forKey:@"gameType"];//[必选]彩票类型 12=六合彩
    [dict setValue:[NSString stringWithFormat:@"%i",_playType] forKey:@"playType"];//[必选]玩法类型

    DLog(@"-=-----%@",dict);
    [[HttpManager manager] requestGetURL:HTTP_markSix_combinaInfo withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
        
        if ([responseObject[@"code"] isEqualToString:@"200"]) {
            
            NSString*betnumber=responseObject[@"data"][@"betNumber"];
            
            if (_bettype==1) {//选择自主下注
                if (_seleteoptionarray.count>0) {
                    [_settlementview setviewlayoutwithbetarray:_seleteoptionarray AndBettype:@"1"];
                    if (_playType==10) {
                        _settlementview.isguoguan=@"1";
                    }else{
                        _settlementview.isguoguan=@"";
                    }
                    _settlementview.betnumber=betnumber;
                    _settlementview.lbtotal.attributedText=_lbfootsett.attributedText;
                    _settlementview.money=_headmodel.money;
            ...
            {//选择快捷下注
                if (_seletequickarray.count>0) {
                    ...
                    _settlementview.money=_headmodel.money;
                    [_settlementview.tfmoney becomeFirstResponder];
                    [_settlementview showView];
                }else{
                    [self presentSheet:@"请选择要下注的内容"];
