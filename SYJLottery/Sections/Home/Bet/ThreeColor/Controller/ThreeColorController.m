...
#import "BetModel.h"
#import "ThreeMovementsView.h"
//#import "DepositViewController.h"
#import "WebViewController.h"

@interface ThreeColorController ()<DropDownViewDelegate,BetFootViewDelegate,BetHeadViewDelegate,UIScrollViewDelegate,BetTypeHeadViewDelegate,SettlementViewDelegate,OptionalViewDelegate,QuickViewDelegate,BetDetailHeadViewDelegate,UITextFieldDelegate>
{
    UIView          *_TopView;               //导航栏
    ...
    int             _playType;              //玩法类型 0=两面盘 1=1-5球
...
@property (nonatomic, strong) ThreeMovementsView    *movementsview;         //走势视图
...
@implementation ThreeColorController
- (instancetype)init {
    self = [super init];
    if (self) {
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [MyConfig currentConfig].isback=@"1";
...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self
                                             selector:@selector(keyboardDidHide:)
                                               ...
                                               object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(isbackrefresh) name:@"thrNotifica" object:nil];
}
//倒计时结束开始请求开奖结果
...
-(void)getcurrentTimedata{
    ...
    DLog(@"----%i",_isrun);
    if (![[MyConfig currentConfig].isback isEqualToString:@"1"]&&_isrun==0) {
        return;
    }
    ...
    [[HttpManager manager] requestGetURL:HTTP_bj3_currentTime withParams:dict withTarget:self withHud:NO withSuccessBlock:^(id responseObject) {
...
-(void)getrankingdata{
    ...
    NSMutableDictionary *dict = [NSMutableDictionary dictionary];
    [dict setValue:[MyConfig currentConfig].userU forKey:@"u"];//[可选]用户key
    [[HttpManager manager] requestGetURL:HTTP_bj3_hotRanking withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
-(void)getbetPaneldata{
    ...
    [[HttpManager manager] requestGetURL:HTTP_bj3_betPane withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        DLog(@"-----= %@", responseObject)
...
-(void)setViewlayout{
    ...
    if (_playType==0) {//两面盘
        ...
        BetPanelModel*onemodel2=[_betpanelarray objectAtIndex:5];
        ...
        _onehead.delegate=self;
        _onehead.viewtag=1;
        _onehead.titleArray=@[@"第1球",@"第2球",@"第3球",@"第4球"];
        [_scrollview addSubview:_onehead];
        
        _twohead=[[BetTypeHeadView alloc]initWithFrame:CGRectMake(0, _onehead.bottom, MSW, BetHeadHight)];
        _twohead.titleArray=@[@"第5球"];
        _twohead.delegate=self;
        _twohead.viewtag=2;
        [_scrollview addSubview:_twohead];
        ...
        for (int i=0; i<4; i++) {
            ...
            BetPanelModel*onemodel=[_betpanelarray objectAtIndex:i];
            NSMutableArray *array = [NSMutableArray array];
            array = [BetModel mj_objectArrayWithKeyValuesArray:onemodel.optionItems];
        ...
        BetPanelModel*onemodel=[_betpanelarray objectAtIndex:4];
        NSMutableArray *array = [NSMutableArray array];
        array = [BetModel mj_objectArrayWithKeyValuesArray:onemodel.optionItems];
        for (int j = 0; j<onemodel.optionItems.count; j++) {
            OptionalView*option = [[OptionalView alloc]initWithFrame:CGRectMake((W)*(j%4),(j/4) * (H), W, H)];
...
#pragma mark - 点击规则
-(void)clickrules{
    [[HttpManager manager] requestGetURL:HTTP_bj3_rules withParams:nil withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
        if ([responseObject[@"code"] isEqualToString:@"200"]) {
...
-(BetDetailHeadView*)headview{
    if (!_headview) {
        ...
        _headview.lblottery.attributedText = [[NSString stringWithFormat:@"北京三分彩<u>   </u><v>000000期</v>"] attributedStringWithStyleBook:style];
...
//走势
-(ThreeMovementsView*)movementsview{
    if (!_movementsview)
    {
        _movementsview=[[ThreeMovementsView alloc]initWithRightdistance:80];
...
-(void)clickbetwithmoney:(NSString *)money{
    ...
    DLog(@"-=-=-----%@-----%@ -------%@-------%@",[MyConfig currentConfig].userU,_headmodel.room,_settlementview.totalmoney,string);
    ...
    if ([_isweb isEqualToString:@"1"]) {
        ...
        NSString *strUrl = [_weburl stringByReplacingOccurrencesOfString:@"t=" withString:@"t=0"];
        ...
        [[HttpManager manager] requestGetURL:HTTP_bj3_bet withParams:dict withTarget:self withHud:YES withSuccessBlock:^(id responseObject) {
            DLog(@"-----= %@=-----%@", responseObject,responseObject[@"msg"]);
